...
const index = () => {
  ...
  return (
   <div className="bg-black mt-[5rem] h-screen">
  <div className="text-gray-300 rounded-lg text-sm w-[100rem] mt-[2rem] ml-[24rem] bg-[#161b22] border border-[#3f3f46] text-center">
    <div className="w-full flex flex-row space-x-3 p-4">
      ...
      <div className="w-full text-gray-500">
        <p className="text-xl text-white">Dao Details</p>
        <p className="w-[200px] text-lg space-x-1 inline-block overflow-hidden whitespace-nowrap text-ellipsis">
          <span className="text-gray-100">DAO Name:{daoInfo.daoName}</span>
          <span className="font-extralight"></span>
        </p>
        {/* <p className="text-sm">DAO Description: {daoInfo.daoDescription}</p> */}
      </div>
    </div>
    <div className="w-full border-b border-[#3f3f46]"></div>
    <div className="w-full flex  space-x-4 p-4 text-gray-500 text-sm">
      <p className="flex justify-between items-center space-x-1">
        <a className="hover:text-blue-500 flex items-center space-x-1" href="#"></a>
        <span className="text-gray-100">DAO_ID: </span>
        <a className="hover:text-blue-500 text-semibold " href="#">
          {Number(daoInfo.daoId)}
            </a>
            <a className="hover:text-blue-500 flex items-center space-x-1" href="#"></a>
        <span className="text-gray-100"> DAO Description: </span>
        <a className="hover:text-blue-500 text-semibold " href="#">
         {daoInfo.daoDescription}
        </a>
      </p>
    </div>
    <div className="w-full border-b border-[#3f3f46]"></div>
    <div className="w-full flex-col space-y-1 p-4 text-gray-500 text-sm">
      <div className="flex items-center space-x-1">
        <p className="text-gray-100"> Total Members : {totalMembers}</p>
      </div>
      ...
      <div className="flex justify-between pt-[4rem]">
        {isMember && (
          <button
            className="border border-indigo-600 bg-black text-white rounded-lg p-4 font-semibold"
    ...
    {isMember && (
      <Flex flexDir={"row"} justifyContent={"center"} alignItems={"center"}  gap={6}>
        <Box>
          <button             className="border border-indigo-600 bg-black text-white rounded-lg p-4 font-semibold mb-[1.5rem]"
 onClick={handleSizeClick5} >Share Files with the DAO members</button>
          <Modal isOpen={isUploadOpen} onClose={onUploadClose} >
            ...
            <ModalContent bg="black">
                  ...
                  >
                    <SimpleGrid columns={1} spacing={6}>
                      <FormControl mr="2%">
                        ...
                        <Input id="name" placeholder="Name" autoComplete="name" onChange={(e) => setDocName(e.target.value)} />
                      </FormControl>
                      <FormControl id="bio">
                        <FormLabel fontSize="sm" fontWeight="md" color="gray.700" _dark={{ color: "gray.50" }}>
                          File description
                        </FormLabel>
                        <Textarea placeholder="Describe the content of the file" rows={3} shadow="sm" focusBorderColor="brand.400" fontSize={{ sm: "sm" }} onChange={(e) => setDocDesc(e.target.value)} />
                        <FormHelperText>Short Description. URLs are hyperlinked.</FormHelperText>
                      </FormControl>
                      <FormControl>
                        <FormLabel fontWeight={"normal"} color="gray.700" _dark={{ color: "gray.50" }}>
                          Attach File
                        </FormLabel>
                        <Flex mt={1} justify="center" px={6} pt={5} pb={6} borderWidth={2} _dark={{ color: "gray.500" }} borderStyle="dashed" rounded="md">
                          <Stack spacing={1} textAlign="center">
                            <Icon mx="auto" boxSize={12} color="gray.400" _dark={{ color: "gray.500" }} stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true">
                            ...
                            <Flex fontSize="sm" color="gray.600" _dark={{ color: "gray.400" }} alignItems="baseline">
                              <chakra.label cursor="pointer" rounded="md" fontSize="md" color="brand.600" _dark={{ color: "brand.200" }} pos="relative" _hover={{ color: "brand.400", _dark: { color: "brand.300" } }}>
                                <span>{"Upload Image"}</span>
                                <VisuallyHidden>
                                  <input id="file-upload" name="file-upload" type="file" ref={inputRef} onChange={changeHandler} accept=".png, .jpg, .jpeg" />
                                </VisuallyHidden>
                              </chakra.label>
                              <Text pl={1}>or drag and drop</Text>
                            </Flex>
                            <Text fontSize="xs" color="gray.500" _dark={{ color: "gray.50" }}>PNG, JPG, JPEG up to 10MB</Text>
                          </Stack>
                        </Flex>
                      </FormControl>
              ...
              <ModalFooter>
                <Button display="block" mx="auto" mt={6} w="10rem" colorScheme="purple" variant="solid" onClick={handleSubmit}>
                  Upload
                </Button>
              </ModalFooter>
            </ModalContent>
 ...
 onClick={() => router.push(`/dao/files/${daoInfo.daoId}`)} >
        ...
        <Box>
          <button             className="border border-indigo-600 bg-black text-white rounded-lg p-4 font-semibold mb-[1.5rem]"
 onClick={() => router.push(`/chat/${process.env.NEXT_PUBLIC_GROUP_ID}`)} >
            DAO Forum
          </button>
        </Box>
      </Flex>
  ...
  <>
        ...
        {propSignal ? (
          <Tabs>
            ...
            <TabPanels>
              <TabPanel>
                {proposalArray
                  .filter((proposal) => filteringDaos(proposal.proposalInfo.beginningTime, proposal.proposalInfo.endingTime) == 0)
                  .map((proposal) => (
                    <Box p={4} borderWidth="1px" borderRadius="lg" position="relative">
                      <Heading size="sm">
                        {proposal.proposalInfo[2].substring(0, proposal.proposalInfo[2].indexOf("|"))}
                      </Heading>
                      <Text mt={2}>
                        {proposal.proposalInfo[2].substring(proposal.proposalInfo[2].indexOf("|") + 1, proposal.proposalInfo[2].length)}
                      </Text>
                      <Flex mt={3}>
                        <span mb={4} h={6}> ⚠️</span>
                        <h3 mt={10}>
                          {Math.floor((proposal.proposalInfo.endingTime.toString() - currentTimestamp) / 3600)} hrs remaining
                        </h3>
                      </Flex>
                      {isMember ? (
                        <Button onClick={() => { setProposalForVote(Number(proposal.proposalInfo.proposalId)); handleSizeClick2(); }} mt={4} colorScheme="teal">
                          Vote Now
                        </Button>
                      ) : null}
                    </Box>
              ...
              <TabPanel>
                {proposalArray
                  .filter((proposal) => filteringDaos(proposal.proposalInfo.beginningTime, proposal.proposalInfo.endingTime) == -1)
                  .map((proposal) => (
                    <Box p={4} borderWidth="1px" borderRadius="lg" position="relative">
                      <Heading size="sm">
                        {proposal.proposalInfo[2].substring(0, proposal.proposalInfo[2].indexOf("|"))}
                      ...
                      <Flex mt={3}>
                        <span mb={4} h={6}> ⚠️</span>
                        <h3 mt={10} ml={4}>
                          {Math.floor((proposal.proposalInfo.beginningTime - currentTimestamp) / 3600)} hrs to start voting
                        </h3>
                      </Flex>
                    </Box>
                  ))}
                  ...
                  .map((proposal) => (
                    <Box p={4} borderWidth="1px" borderRadius="lg" position="relative">
                      ...
                      <Flex mt={3}>
                        <span mb={4} h={6}> ⚠️</span>
                        <h3 mt={10}>
                          Expired {Math.floor((currentTimestamp - proposal.proposalInfo.endingTime.toString()) / 3600)} hrs ago
                        </h3>
                      </Flex>
                      <Button onClick={() => { getVotingResults(Number(proposal.proposalInfo.proposalId)); handleSizeClick4(); }} mt={4} colorScheme="teal">
                        View Results
                      </Button>
                    </Box>
                  ))}
      ...
      <ModalContent bg="black">
        ...
        <ModalBody>
          <FormControl mr="5%" isRequired>
            ...
            <Input id="subject" placeholder="Enter Proposal Title" onChange={(e) => setTitle(e.target.value)} />
          </FormControl>
          <FormControl mt={1} isRequired>
            <FormLabel fontSize="sm" fontWeight="md" color="gray.700" _dark={{ color: "gray.50" }}>
              Proposal Description
            </FormLabel>
            <Textarea placeholder="Write a short description for proposal" rows={3} shadow="sm" focusBorderColor="brand.400" fontSize={{ sm: "sm" }} onChange={(e) => setDescription(e.target.value)} />
          ...
          <FormControl mr="2%" mt="2%" isRequired>
            ...
            <Input id="threshholdToken" placeholder="Enter Minimum tokens required to vote" autoComplete="email" onChange={(e) => setVotingThreshold(e.target.value)} />
          </FormControl>
          <FormControl mr="2%" mt="2%" isRequired>
            <FormLabel htmlFor="name" fontWeight={"normal"}>
              Passing Threshold
            </FormLabel>
            <Input id="threshholdToken" placeholder="Enter Minimum tokens required to pass proposal" autoComplete="email" onChange={(e) => setPassingThreshold(e.target.value)} />
            <FormHelperText>Enter minimum number of votes to pass a proposal</FormHelperText>
          </FormControl>
          <FormControl mr="2%" mt="2%" isRequired>
            <FormLabel htmlFor="name" fontWeight={"normal"}>
              Token Address
            </FormLabel>
            <Input id="tokenAddress" placeholder="Token address of proposal token" onChange={(e) => setTokenAddress(e.target.value)} />
          </FormControl>
          <FormControl mr="5%">
            <FormLabel htmlFor="datetime-local" fontWeight={"normal"} isRequired>
              Voting Start Date
            </FormLabel>
            <Input placeholder="Select Date " size="md" type="datetime-local" id="datetime-local" onChange={(e) => { setStartDate(convertString(e.target.value) as any); }} />
          </FormControl>
          <FormControl mr="5%">
            <FormLabel htmlFor="datetime-local" fontWeight={"normal"} isRequired>
              Voting End Date
            </FormLabel>
            <Input placeholder="Select Date and Time" size="md" type="datetime-local" id="datetime-local" onChange={(e) => { setEndTime(convertString(e.target.value) as any); }} />
          </FormControl>
          <FormControl mr="5%" mt="3%" isRequired>
            ...
            <RadioGroup defaultValue="2">
              <Stack spacing={5} direction="row">
                <Radio colorScheme="red" value="1" onChange={() => setvoteOnce(false)}>
                  No
                </Radio>
                <Radio colorScheme="green" value="2" onChange={() => setvoteOnce(true)}>
                  Yes
                </Radio>
              </Stack>
            </RadioGroup>
          ...
          <FormControl mr="5%" mt="3%" isRequired>
            <FormLabel htmlFor="specialization" fontSize="sm" fontWeight="md" color="gray.700" _dark={{ color: "gray.50" }}>
              Proposal Type
            </FormLabel>
            <Select id="specialization" name="specialization" autoComplete="specialization" placeholder="Select option" focusBorderColor="brand.400" shadow="sm" size="sm" w="full" rounded="md" onChange={(e) => setProposalType(e.target.value)}>
    ...
    <Modal isOpen={isVoteOpen} onClose={onVoteClose}>
      ...
      <ModalContent>
        ...
        <ModalCloseButton />
        <ModalBody>
          <Select onChange={(e) => { setUserResponse(e.target.value); }} placeholder="Select option">
        ...
        <Text ml={7} mt={2}>
          Please Authorize first and wait for the transaction to end. Then press Submit
        </Text>
        <ModalFooter>
          <Button onClick={() => { console.log(userResponse); console.log(proposalForVote); authorizeContract(); }} colorScheme="orange" m={2}>
            Authorize & Vote
          </Button>
        </ModalFooter>
      </ModalContent>
    ...
    <Modal isOpen={isStartOpen} onClose={onStartClose}>
      ...
      <ModalContent bg='black'>
        ...
        <ModalCloseButton />
        <ModalBody>
          <Input type="text" placeholder="Add wallet address of the user you want to invite to DAO" onChange={(e) => { setInviteAddress(e.target.value); }} />
        </ModalBody>
        <ModalFooter>
